...
__global__ void point_iterate(iint* area, const complex z1, const complex z2, const int nx, const int ny, const int max_iter) {
    int i = 0;
    std::complex z = 0.0;
    ...
    std::complex z = z2 - z1 ;
    z.real(z.real() * ix / (nx - 1.0));
    z.imag(z.imag() * iy / (ny - 1.0));
    std::complex c = z1 + z;
...
int main(int argc, char *argv[]) {
    ...
    h_m = (int *)malloc(sizeof(int)*rows*cols);
    ...
    cuda_hello<<< grid, block >>>(d_m, z1, z2, cols, rows, 100);
